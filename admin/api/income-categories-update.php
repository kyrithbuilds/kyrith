...
try {
    ...
    if ($isActive === 0) {
        $usage = $pdo->prepare('SELECT COUNT(*) FROM income WHERE category_id = :id');
        $usage->execute(['id' => $id]);
    ...
    $stmt = $pdo->prepare('UPDATE income_categories SET name = :name, is_active = :is_active WHERE id = :id');
    $stmt->execute(['id' => $id, 'name' => $name, 'is_active' => $isActive]);

    activity_log($pdo, $adminUserId, 'updated_income_category', 'income_categories', $id, ['name' => $name]);
