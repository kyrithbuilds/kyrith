...
if ($id <= 0 || $name === null || $amount === null || !validate_date_ymd($incomeDate)) {
...
try {
    ...
    $exists = $pdo->prepare('SELECT id FROM income WHERE id = :id');
    $exists->execute(['id' => $id]);
    if (!$exists->fetch()) {
        http_response_code(404);
        echo json_encode(['ok' => false, 'error' => 'Income record not found.']);
        exit;
    }
    ...
    $stmt = $pdo->prepare(
        'UPDATE income SET
            ...
            received_by_partner_id = :received_by_partner_id
         WHERE id = :id'
    );
    $stmt->execute([
        'id' => $id,
    ...
    activity_log($pdo, $adminUserId, 'updated_income', 'income', $id, [
        'name' => $name,
        'amount' => $amount,
        'income_date' => $incomeDate,
    ]);
    ...
    echo json_encode(['ok' => false, 'error' => 'Could not update income. Please try again.']);
}
